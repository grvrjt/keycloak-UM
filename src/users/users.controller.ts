...
export class UsersController {
  ...
  @Post('/assign-user-role/:userId')
  @Unprotected()
  async assignRoleToUser(
    @Param('userId') userId: string,
    @Body() roleDetail: { id: string, name: string, description: string }) {
    const response = await this.UserService.assignRoleToUser(userId, roleDetail)
    return {
      success: response,
